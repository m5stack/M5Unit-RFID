...
TEST_P(TestWS1850S, coporcessorCRC) {
    ...
    for (auto&& e : table) {
        ...
        uint16_t result{};
        EXPECT_TRUE(unit->calculateCRC(tdata.data(), tdata.size(), result)) << e.name;

        uint16_t cr = crc.range(tdata.data(), tdata.size());
...
TEST_P(TestWS1850S, Antenna) {
    ...
    // to ON
    EXPECT_TRUE(unit->readRegister8(m5::unit::mfrc522::command::TX_CONTROL_REG, prev, 0));

    EXPECT_TRUE(unit->turnOffAntenna());
    EXPECT_TRUE(unit->readRegister8(m5::unit::mfrc522::command::TX_CONTROL_REG, now, 0));
    EXPECT_TRUE(unit->isAntennaOn(onoff));
    EXPECT_FALSE(onoff);
    EXPECT_NE(now, prev);
    prev = now;
    ...
    EXPECT_TRUE(unit->readRegister8(m5::unit::mfrc522::command::RFC_FG_REG, prev, 0));

    for (auto&& e : table) {
        EXPECT_TRUE(unit->setAntennaGain(e)) << (int)e;
        EXPECT_TRUE(unit->readRegister8(m5::unit::mfrc522::command::RFC_FG_REG, now, 0));
